...
--
-- This migration:
--   1. Adds customer lifetime-value columns (total_purchase_amount,
--      total_purchase_count, pending_balance)
--   2. Creates an audit_logs table for all write operations
--   3. Creates the create_invoice() SECURITY DEFINER RPC that
--      atomically: inserts bill + items, deducts inventory (with
--      negative-stock prevention), updates customer stats, records
--      payment if money received, stores paint shade history with
--      optional room/house-mapping, and writes an audit entry.
--   4. Grants execute to the authenticated role
-- ============================================================
...
-- function (which bypasses RLS) can write audit rows.
CREATE POLICY "audit_logs: service insert"
  ON public.audit_logs FOR INSERT
  WITH CHECK (false);

-- ─── 3. create_invoice() RPC ─────────────────────────────────
...
BEGIN
  ...
  -- We lock the rows (FOR UPDATE) so concurrent invoices cannot
  -- race past this check on the same product.
  FOR v_item IN SELECT * FROM jsonb_array_elements(p_payload->'items') LOOP

    SELECT i.quantity INTO v_inv_qty
      FROM public.inventory i
     WHERE i.product_id = (v_item->>'product_id')::uuid
       FOR UPDATE;               -- row-level lock for this txn

    IF NOT FOUND THEN
      RAISE EXCEPTION 'STOCK: No inventory record exists for product "%". Add one first.',
  ...
  -- ── 3. Calculate bill totals (loop 1 of 2) ───────────────
  FOR v_item IN SELECT * FROM jsonb_array_elements(p_payload->'items') LOOP
  ...
  -- Cap paid_amount at grand total (guard against over-payment)
  v_paid_amount := LEAST(v_paid_amount, v_grand_total);
  ...
  -- ── 5. Insert items  +  deduct inventory  +  shade history
  --       (loop 2 of 2)  ─────────────────────────────────────
  FOR v_item IN SELECT * FROM jsonb_array_elements(p_payload->'items') LOOP
    ...
    -- 5a. Bill item row (snapshot product details at sale time)
    ...
    -- 5b. Deduct inventory
    --     The CHECK (quantity >= 0) on the inventory table acts as a final
    --     safety net in case the pre-validation above was somehow bypassed.
    UPDATE public.inventory
       SET quantity = quantity - (v_item->>'quantity')::numeric
     WHERE product_id = (v_item->>'product_id')::uuid;

    -- 5c. Paint shade history
    --     Record an entry for any item that has a shade_number.
  ...
  -- ── 7. Payment record (only when money was received) ─────
  IF v_paid_amount > 0 THEN
    -- Map 'credit' → 'other' because payments table only accepts:
...
EXCEPTION
  WHEN check_violation THEN
    -- Triggered if inventory.quantity would go below 0 (safety net)
  ...
  WHEN unique_violation THEN
    RAISE EXCEPTION 'DUPLICATE: A unique constraint was violated. Transaction rolled back.'
      USING ERRCODE = 'P0006';
  WHEN OTHERS THEN
    RAISE;  -- re-raise unchanged so caller gets the original message
END;
$$;
...
COMMENT ON FUNCTION public.create_invoice(jsonb) IS
  ...
  'updates customer stats, records payment if paid_amount > 0, '
  'stores paint shade history for shaded items, and writes an audit log entry. '
  'Entire operation rolls back on any error.';
